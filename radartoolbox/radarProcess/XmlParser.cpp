...
std::string::iterator XmlParser::begin ()
{
	std::string::iterator buf = static_cast<std::string::iterator>(_buffer);
	return std::string::iterator(buf);
}

std::string ::iterator XmlParser::end ()
{
	std::string::iterator buf = static_cast<std::string::iterator>(_buffer + _parseLength);
	return std::string::iterator(buf);
}
...
long XmlParser::find ( char srchChar, long offset, long length)
{
	...
	std::string::iterator start = static_cast<std::string::iterator>(_buffer + offset);
	std::string::iterator end   = static_cast<std::string::iterator>(_buffer + (offset + length));

	// search for it
	std::string::iterator found = std::find( start, end, srchChar );
...
long XmlParser::find ( char * srchStr, long offset, long length)
{
	...
	std::string::iterator srchStart = static_cast<std::string::iterator>(srchStr);
	std::string::iterator srchEnd   = static_cast<std::string::iterator>(srchStr + strlen(srchStr));

	// search for it
	std::string::iterator found = std::search( start, end, srchStart, srchEnd );
...
long XmlParser::find ( std::string & srchStr, long offset, long length)
{
	...
	std::string::iterator srchStart = srchStr.begin();
	std::string::iterator srchEnd   = srchStr.end();
...
long XmlParser::rfind ( char srchChar, long offset, long length )
{
	// setup srch string
	char srchStr[2];
	srchStr[0] = srchChar;
...
long XmlParser::rfind ( char * srchStr, long offset, long length )
{
	/*
	// set start and end of search 
